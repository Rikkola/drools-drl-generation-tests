...
├── drlgen-tests/          # Core testing framework and generation services
...
| Module | Description |
|--------|-------------|
| `drlgen-tests` | Core library with generation agents, validation services, and test scenarios |
...
```bash
# Run default tests (uses qwen3-coder-next)
mvn test -pl drlgen-tests

# Run with a specific model
mvn test -pl drlgen-tests -Dtest.ollama.model=granite4:small-h
...
```bash
# DRL generation tests
mvn test -pl drlgen-tests -Dtest=DRLGenerationTest

# YAML rule generation tests
mvn test -pl drlgen-tests -Dtest=YAMLRuleGenerationTest

# Plain English generation tests
mvn test -pl drlgen-tests -Dtest=PlainEnglishGenerationTest
```
...
| Profile | Description |
|---------|-------------|
| (default) | Quick tests, single model |
| `model-comparison` | Multi-model comparison tests |
| `all-tests` | All tests including slow ones |
